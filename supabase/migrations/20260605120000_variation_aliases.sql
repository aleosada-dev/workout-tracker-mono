...
-- ----------------------------------------------------------------------------
-- wt_get_exercise_history: agora recebe p_alias_id opcional para escopar a tela
-- de detalhes ao contexto da execução (a máquina/alias selecionado).
--   - p_alias_id NULL (entrada pela lista de exercícios): histórico de TODAS as
--     sessões e o record geral (linha alias_id IS NULL).
--   - p_alias_id informado (entrada pela execução): só as sessões daquele alias e
--     o record daquele alias.
-- Sem o filtro, a subquery de records voltava várias linhas (geral + por alias) e
-- estourava 21000 (cardinality_violation). A assinatura muda (2 -> 3 args), então
-- dropamos a antiga antes de recriar.
-- ----------------------------------------------------------------------------
DROP FUNCTION IF EXISTS "public"."wt_get_exercise_history"("uuid", "uuid");
...
ALTER FUNCTION "public"."wt_get_exercise_history"("p_user_id" "uuid", "p_variation_id" "uuid", "p_alias_id" "uuid") OWNER TO "postgres";
GRANT ALL ON FUNCTION "public"."wt_get_exercise_history"("p_user_id" "uuid", "p_variation_id" "uuid", "p_alias_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."wt_get_exercise_history"("p_user_id" "uuid", "p_variation_id" "uuid", "p_alias_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."wt_get_exercise_history"("p_user_id" "uuid", "p_variation_id" "uuid", "p_alias_id" "uuid") TO "service_role";
