jest.mock('@/workout-logs/hooks/use-workout-log-summaries', () => ({
  useWorkoutLogSummaries: jest.fn(),
  PAGE_SIZE: 10,
}));

jest.mock('react-native-toast-message', () => {
  const { createToastMock } = jest.requireActual('@/test-utils/react-native-toast-message');
...
jest.mock('@shopify/flash-list', () => {
  const { createFlashListMock } = jest.requireActual('@/test-utils/shopify-flash-list');
  return createFlashListMock();
});

jest.mock('react-i18next', () => {
  const { createI18nMock } = jest.requireActual('@/test-utils/react-i18next');
...
import { ApiError, ApiUnauthorizedError } from '@/api/lib/errors';
import { observability } from '@/observability/lib';
